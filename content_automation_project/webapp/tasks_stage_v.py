...
def run_step1_job(job_id: str, pair_indices: Optional[List[int]] = None) -> None:
    ...
    try:
        ...
        if jt == "table_notes":
            ...
            return
        if jt == "importance_type":
            db.close()
            from webapp.tasks_single_stage import run_importance_type_step1_job
        ...
        if jt == "flashcard":
            ...
            return
        if jt == "chapter_summary":
            db.close()
            from webapp.tasks_single_stage import run_chapter_summary_step1_job
