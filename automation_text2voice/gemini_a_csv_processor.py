...
class GeminiACSVProcessor:
    ...
    def find_csv_files(self, chapter_number: str) -> str:
        """Find CSV file based on chapter number pattern - only 'a' files"""
        study_file = None
        
        if not self.input_folder_path or not os.path.exists(self.input_folder_path):
            return None
        
        # Look for files starting with 'a' + chapter_number only
    ...
    def setup_logging(self):
        ...
        log_filename = f"gemini_a_processor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
    ...
    def generate_response(self, prompt: str, api_key_data: Dict, chapter_number: str) -> Optional[str]:
        """Generate response from Gemini API with CSV file"""
        try:
            # Setup model with current API key
            if not self.setup_gemini_model(self.model_name, api_key_data['api_key']):
                return None
            
            # Find CSV file based on chapter number (only 'a' files)
            study_file_path = self.find_csv_files(chapter_number)
            
            # Read CSV file and include its content in the prompt
    ...
    def save_response_to_txt(self, topic_id: str, response: str, output_dir: str):
        """Save response to TXT file named with m_ prefix and 6-digit topic number"""
        try:
            # Extract 6-digit topic number from TopicID
            numeric_part = ''.join(filter(str.isdigit, topic_id))
            if len(numeric_part) >= 6:
                topic_number = numeric_part[:6]
            else:
                # Pad with zeros if less than 6 digits
                topic_number = numeric_part.ljust(6, '0')
            
            filename = f"m_{topic_number}.txt"
            ...
            # Save as plain text file with UTF-8 encoding
            with open(filepath, 'w', encoding='utf-8') as file:
                file.write(cleaned_response)
            
            self.logger.info(f"Saved TXT response to {filepath}")
            
        except Exception as e:
            self.logger.error(f"Error saving response to TXT: {str(e)}")
        ...
        for index, row in enumerate(self.output_file_data):
            try:
                ...
                if response:
                    ...
                    self.save_response_to_txt(topic_id, response, output_dir)
...
class GeminiAProcessorGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Gemini A-CSV Processor")
        self.root.geometry("600x500")
        
        self.processor = GeminiACSVProcessor()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # Info label
        ttk.Label(main_frame, text="Note: App will auto-find CSV files with pattern 'a[chapter].csv' only", 
