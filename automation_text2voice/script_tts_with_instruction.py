...
class ScriptTTSProcessor:
    ...
    async def generate_tts_async(self, text: str, output_file: str, voice: str, model: str, api_key: str, instruction: str = ""):
    ...
    def generate_tts(self, text: str, output_file: str, voice: str, model: str, api_key: str, instruction: str = "") -> bool:
        ...
        try:
            ...
            try:
                return loop.run_until_complete(self.generate_tts_async(text, output_file, voice, model, api_key, instruction))
            finally:
                loop.close()
...
class ScriptTTSGUI:
    def __init__(self, root):
        ...
        self.instruction_file_var = tk.StringVar()
    ...
    def setup_ui(self):
        ...
        ttk.Label(main_frame, text="Instruction Prompt File:").grid(row=3, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.instruction_file_var, width=50).grid(row=3, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(main_frame, text="Browse", command=self.browse_instruction_file).grid(row=3, column=2, padx=5)
        
        # Model selection
        ttk.Label(main_frame, text="TTS Model:").grid(row=4, column=0, sticky=tk.W, pady=5)
        model_frame = ttk.Frame(main_frame)
        model_frame.grid(row=4, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        # Process button
        ttk.Button(main_frame, text="Start TTS Processing", command=self.start_processing).grid(row=5, column=0, columnspan=3, pady=20)
        ...
        self.log_text.grid(row=8, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=10)
    ...
    def browse_instruction_file(self):
        file = filedialog.askopenfilename(
            title="Select Instruction Prompt File",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")]
        )
        if file:
            self.instruction_file_var.set(file)
    ...
    def validate_inputs(self) -> bool:
        ...
        # Instruction file is optional, but if provided, it must exist
        if self.instruction_file_var.get() and not os.path.exists(self.instruction_file_var.get()):
            messagebox.showerror("Error", "Instruction prompt file does not exist")
            return False
            
        return True
    
    def load_instruction_file(self) -> str:
        """Load instruction prompt from file"""
        if not self.instruction_file_var.get():
            return ""
            
        try:
            with open(self.instruction_file_var.get(), 'r', encoding='utf-8') as file:
                instruction = file.read().strip()
                self.log_message(f"Loaded instruction prompt ({len(instruction)} characters)")
                return instruction
        except Exception as e:
            self.log_message(f"Warning: Failed to load instruction file: {str(e)}")
            return ""
    ...
    def load_files(self) -> bool:
        ...
        try:
            ...
            self.instruction_prompt = self.load_instruction_file()
        ...
        try:
            ...
            for group_key, group_data in groups.items():
                ...
                if self.instruction_prompt:
                    self.log_message(f"  Using instruction prompt ({len(self.instruction_prompt)} characters)")
